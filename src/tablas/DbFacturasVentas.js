...
export default function DbFacturasVentas(props) {
  ...
  useEffect(() => {
    ...
    db.collection("peluches").onSnapshot((querySnapshot)=>{
      const arreglo = [];
      const ids = [];
  ...
  <div className="paginacion">
    ...
    ):<a href={"/facturasVentas"+"/"+(Number(p)-1)}><i  className="fa fa-chevron-left "></i></a>}
    
       <p>{p}</p>
      
       {(Number(p) == Ind.length)?(
       <p className="disabled"><i  className="fa fa-chevron-right"></i></p>
    ):<a href={"/facturasVentas"+"/"+(Number(p)+1)}><i  className="fa fa-chevron-right "></i></a>}
  </div>
